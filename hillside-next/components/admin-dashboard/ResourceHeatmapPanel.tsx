...
function toneForValue(value: number) {
  if (value >= 80) return "bg-[var(--color-secondary)] text-white";
  ...
  return (
    <section className="surface p-5 sm:p-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[var(--color-secondary)]">Resource Heatmap</p>
          <h2 className="mt-2 text-xl font-bold text-[var(--color-text)] lg:text-2xl">Team load by day</h2>
          <p className="mt-1 text-sm text-[var(--color-muted)]">Plan housekeeping, kitchen &amp; front-desk staffing.</p>
        </div>
        <span className="shrink-0 rounded-full border border-[var(--color-border)] px-3 py-1 text-xs font-semibold text-[var(--color-text)]">
          Avg {snapshot?.ai_demand_7d.avg_occupancy_pct ?? 0}%
        </span>
      </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {TEAMS.map((team) => {
                  ...
                  return (
                    <tr key={team.key} className="border-t border-[var(--color-border)] bg-white">
