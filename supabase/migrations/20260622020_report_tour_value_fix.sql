-- Fix: tour_booked_value was counting tours from CANCELLED reservations, because
-- cancelling a reservation does not cascade to service_bookings.status (it stays
-- 'pending_payment'). The tour CTE filtered on the service-booking status, so it
-- never saw the parent reservation was cancelled. Now it joins to reservations
-- and filters on the RESERVATION status, exactly like unit_booked_value does.
-- Body-only change (same return type), so a plain CREATE OR REPLACE. Named tag.
