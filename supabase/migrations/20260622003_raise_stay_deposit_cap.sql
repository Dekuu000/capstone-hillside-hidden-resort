-- Raise the stay deposit cap so large / many-guest bookings deposit proportionally.
-- Policy: deposit = 20% of total, floor ₱500, cap raised ₱1,000 -> ₱5,000 (still
-- never more than the total). Pax-priced units (Pavilion / Exclusive) already grow
-- the total with guest count, so 20% now scales the deposit with group size instead
-- of flattening every big booking to ₱1,000.
--
-- Only the deposit clamp + policy labels change vs. 20260531002; the rest of the
-- function body is reproduced verbatim so this CREATE OR REPLACE stays faithful.
...
BEGIN
  ...
  -- Deposit: 20% of total, floored at ₱500, capped at ₱5,000, never above total.
