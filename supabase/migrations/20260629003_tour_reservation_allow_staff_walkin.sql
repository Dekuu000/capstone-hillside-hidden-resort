-- ============================================
-- Allow Front Desk (staff) to create same-day walk-in tour reservations
-- Created: 2026-06-29
-- Same RBAC gap as record_on_site_payment / perform_checkin (20260629001/002):
-- create_tour_reservation_atomic only recognized role = 'admin' as resort staff,
-- so Front Desk (role 'staff') creating a same-day walk-in tour (p_is_advance =
-- false) was treated as a guest and rejected with "Guests can only create advance
-- tour reservations". The API already enforces the correct model (guests ->
-- advance only; operations -> walk-in only), so this only blocked at the database.
--
-- Recognize the operations tier (staff, admin, super_admin) as resort staff:
--   * only true guests are restricted to advance reservations;
--   * the operations tier is the one barred from advance/online (use walk-in).
-- Body otherwise identical to 20260622013_tour_rpc_fn.sql.
-- ============================================
...
BEGIN
  ...
  v_is_operations := v_role IN ('staff', 'admin', 'super_admin');

  IF NOT v_is_operations AND p_guest_user_id != auth.uid() THEN
    RAISE EXCEPTION 'Not authorized to create this reservation';
  END IF;

  -- Resort staff (operations tier) create same-day walk-in tours, not advance
  -- reservations; advance/online bookings are the guest path.
  IF v_is_operations AND p_is_advance IS TRUE THEN
    RAISE EXCEPTION 'Resort staff create same-day walk-in tour reservations. Use the walk-in flow.';
  END IF;

  IF NOT v_is_operations AND p_is_advance IS DISTINCT FROM TRUE THEN
  ...
  -- Resolve promo (entered code or best auto-apply for tours) before the deposit.
  SELECT promo_id, code, discount
  INTO v_promo_id, v_promo_code, v_discount
  FROM public.resolve_promo_discount(p_promo_code, v_total, p_guest_user_id, 'tours');
  ...
  INSERT INTO public.reservations (
    ...
    v_original_total,
    v_discount,
    CASE WHEN v_discount > 0 THEN v_promo_code ELSE NULL END,
    v_deposit,
    v_expected_pay_now,
    0,
    'pending_payment',
  ...
  IF v_discount > 0 AND v_promo_id IS NOT NULL THEN
    ...
    VALUES (v_promo_id, v_reservation_id, p_guest_user_id, v_discount);
  END IF;
  ...
  PERFORM public.create_audit_log(
    ...
    jsonb_build_object(
      ...
      'promo_code', CASE WHEN v_discount > 0 THEN v_promo_code ELSE NULL END,
  ...
  RETURN QUERY SELECT
    ...
    NULL::TEXT;
END;
$tour_fn$ LANGUAGE plpgsql SECURITY DEFINER;
