-- Cancelling a reservation now cascades 'cancelled' to its linked service/tour
-- bookings, so downstream reads (service queues, any sb.status consumers) stay
-- consistent with the reservation. Body-only change (RETURNS VOID, same args),
-- so a plain CREATE OR REPLACE preserves the existing grant. Named tag.

CREATE OR REPLACE FUNCTION public.cancel_reservation(
  p_reservation_id UUID
) RETURNS VOID AS $cancel_res$
DECLARE
...
BEGIN
  ...
  -- Cascade to linked tour/service bookings so their status matches the
  -- reservation (leave terminal check-in states untouched, which can't happen
  -- here anyway since cancel is blocked after check-in).
  UPDATE public.service_bookings
  SET status = 'cancelled'
  WHERE reservation_id = p_reservation_id
    AND status NOT IN ('cancelled', 'no_show', 'checked_in');
...
END;
$cancel_res$ LANGUAGE plpgsql SECURITY DEFINER;
