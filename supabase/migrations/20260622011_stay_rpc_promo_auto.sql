-- Stay reservation RPC now resolves the promo via resolve_promo_discount, so it
-- supports both entered codes AND auto-apply seasonal promos. No signature change
-- vs 20260622006, so a plain CREATE OR REPLACE. Named dollar-quote tag; no dollar
-- tokens in comments.
...
BEGIN
  ...
  -- Resolve promo (entered code or best auto-apply) and apply before the deposit.
  SELECT promo_id, code, discount
  INTO v_promo_id, v_promo_code, v_discount
  FROM public.resolve_promo_discount(p_promo_code, v_total, p_guest_user_id, 'stays');
  ...
  RETURN QUERY SELECT
    ...
    NULL::TEXT;
END;
$stay_fn$ LANGUAGE plpgsql SECURITY DEFINER;
