-- Prevent admin accounts from creating online (advance) tour reservations.
-- Admin tour creation must use walk-in flow only (is_advance = false).
...
BEGIN
  ...
  -- Admin must use walk-in tour flow only.
